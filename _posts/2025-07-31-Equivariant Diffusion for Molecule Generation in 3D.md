...
In the **forward diffusion process** , a molecule's atom positions xxx and atom types hhh are corrupted by adding Gaussian noise at each time step. This process continues until the data becomes indistinguishable from random noise. For example, for time step ttt, the noisy latent representation is given by:
...
A practical trick that improves both training and sample quality is to train the model to **predict the noise ε\epsilonε** added at each step, rather than the denoised data xxx itself. This simplification, introduced by Ho et al. (2020), leads to a cleaner optimization objective:
...
- Coordinate updates use relative vectors $x_i - x_j$ , preserving rotational and translational equivariance. EGNN layers are made of:
...
In practice, _$L_t$_ dominates training and performs well when the weighting factor _w(t)_ is set to 1 (even though the theory suggests a time-dependent weight based on the signal-to-noise ratio).
